...
- `src/styles/puzzle.css` — all twelve puzzle games (Jigsaw, Slide, Connect-the-Dots, Maze,
  Spot-the-Difference, Shape Fit, Color Sort, Memory Match, Shadow Match, Pattern Sequence,
  Word Search, Pipe Connector). Kept separate from `games-shared.css` so each file is
